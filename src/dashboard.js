...
const convertTZ = tzString => date => new Date((typeof date === "string" ? new Date(date) : date).toLocaleString("en-US", {timeZone: tzString}))
...
const openNow = facility => Some(`${facility.name}: open now, closes at ${facility.close} pm`)
...
const maybeOpenByHour = (time, facility) =>
  isAM(time) ? 
  (time.hour < getOpenHour(facility) ? facilityClosed(facility) : openNow(facility)) :
  (time.hour >= getClosedHour(facility) ? facilityClosed(facility) : openNow(facility))
...
const unwrapTransferStationHours = compose(x => x.contents(),x => x.fold(id))
const getTransferStationHours = compose(unwrapTransferStationHours,getFacilityHours)
